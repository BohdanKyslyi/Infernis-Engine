#!/usr/bin/env python3
"""Switch Infernis PBR Stage 2.45 binary-refined HUD SSLR modes."""
...
NAMES = {
    (0, 0, 0): "baseline",
    (1, 1, 1): "binary-refined projection hit mask",
    (2, 1, 1): "binary-refined scene-visible HUD SSLR",
}
...
def main() -> None:
    if not COMBINE.is_file() or not HEADER.is_file():
        raise SystemExit("Stage 2.45 shader files were not found")

    if len(sys.argv) != 2 or sys.argv[1] in {"-h", "--help"}:
        raise SystemExit(
            f"Usage: py {Path(__file__).name} "
            "[baseline|stable_mask|stable_sslr|production|status]"
        )
    ...
    if command == "status":
        state = (
            read_value(SSLR_RE, combine, "Stage 2.43"),
            read_value(PROJECTION_RE, combine, "Stage 2.44"),
            read_value(BINARY_RE, combine, "Stage 2.45"),
        )
        print(f"Stage 2.45: {NAMES.get(state, 'custom state')}")
        return
    if command not in MODES:
        raise SystemExit(f"Unknown mode: {command}")

    sslr_mode, projection, binary, sky_mode = MODES[command]
    ...
    print(f"Stage 2.45: {NAMES[(sslr_mode, projection, binary)]}")
    print("Stage 2.42 was restored to baseline; sky reflection remains fallback.")
