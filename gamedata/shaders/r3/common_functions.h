...
float gbuf_pack_pbr(float hemiAO, float metallic)
{
    ...
    uint m =
        (uint)(saturate(metallic) * 31.0f + 0.5f);
...
gbuffer_data gbuffer_load_data( float2 tc : TEXCOORD, float2 pos2d, int iSample )
{
 ...
 if (gbuf_is_pbr(P.w))
 {
     ...
     // Temporary fallback for old X-Ray lighting.
     // GGX will replace this later.
     gbd.mtl = 0.5f;
 }
 else
 {
