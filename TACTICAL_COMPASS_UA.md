...
Маркери можна вимкнути окремо (`show_markers="0"`). Наразі підтримуються типи точок карти, назви яких містять `task_location` / `quest_location`, `treasure` / `stash`, `level_changer` / `transition`. Для офлайн-цілей відображається лише позиція на поточному рівні, якщо вона доступна в `CMapLocation`. Кількість одночасних маркерів обмежена 16, список перевіряється раз на 200 мс; самі позиції й шкала оновлюються разом із камерою.
