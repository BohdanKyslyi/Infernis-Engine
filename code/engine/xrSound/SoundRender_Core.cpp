...
void CSoundRender_Core::_clear() {
    ...
    for (u32 sit = 0; sit < s_sources.size(); sit++) xr_delete(s_sources[sit]);
    s_sources.clear();

    for (u32 eit = 0; eit < s_emitters.size(); eit++) xr_delete(s_emitters[eit]);
    s_emitters.clear();
...
void CSoundRender_Core::stop_emitters() {
    for (u32 eit = 0; eit < s_emitters.size(); eit++) s_emitters[eit]->stop(FALSE);
}
...
void CSoundRender_Core::env_unload() {
    if (s_environment) s_environment->Unload();
...
void CSoundRender_Core::set_geometry_som(IReader* I) {
    ...
    struct SOM_poly { Fvector3 v1; Fvector3 v2; Fvector3 v3; u32 b2sided; float occ; };

#ifdef _EDITOR
    CDB::Collector* CL = ETOOLS::create_collector();
    while (!geom->eof()) {
        SOM_poly P; geom->r(&P, sizeof(P));
        ETOOLS::collector_add_face_pd(CL, P.v1, P.v2, P.v3, *(u32*)&P.occ, 0.01f);
        if (P.b2sided) ETOOLS::collector_add_face_pd(CL, P.v3, P.v2, P.v1, *(u32*)&P.occ, 0.01f);
    }
    ...
    while (!geom->eof()) {
        ...
        if (P.b2sided) CL.add_face_packed_D(P.v3, P.v2, P.v1, *(u32*)&P.occ, 0.01f);
    }
...
void CSoundRender_Core::set_geometry_env(IReader* I) {
    ...
    hdrCFORM H; geom->r(&H, sizeof(hdrCFORM));
    Fvector* verts = (Fvector*)geom->pointer();
    CDB::TRI* tris = (CDB::TRI*)(verts + H.vertcount);
    for (u32 it = 0; it < H.facecount; it++) {
        CDB::TRI* T = tris + it;
        u16 id_front = (u16)((T->dummy & 0x0000ffff) >> 0); 
        u16 id_back = (u16)((T->dummy & 0xffff0000) >> 16); 
...
#endif
    geom_ch->close(); geom->close(); xr_free(_data);
}
...
void CSoundRender_Core::attach_tail(ref_sound& S, const char* fName) {
    ...
    if (S._feedback()) ((CSoundRender_Emitter*)S._feedback())->fTimeToStop += s->length_sec();
...
void CSoundRender_Core::play(ref_sound& S, CObject* O, u32 flags, float delay) {
    ...
    if (flags & sm_2D || S._handle()->channels_num() == 2) S._feedback()->switch_to_2D();
}
...
void CSoundRender_Core::play_with_delay(ref_sound& S, CObject* O, const Fvector& pos, u32 flags) {
    ...
    ds.pos = pos;
    ds.flags = flags;
    ds.play_time = fTimer_Value + delay; 

    m_delayed_sounds.push_back(ds);
}

void CSoundRender_Core::update_delayed_sounds() {
    if (!bPresent || m_delayed_sounds.empty()) return;

    float current_time = fTimer_Value;

    for (u32 i = 0; i < m_delayed_sounds.size(); ) {
        if (current_time >= m_delayed_sounds[i].play_time) {
            play_at_pos(m_delayed_sounds[i].snd, m_delayed_sounds[i].obj, m_delayed_sounds[i].pos, m_delayed_sounds[i].flags, 0.f);
            m_delayed_sounds.erase(m_delayed_sounds.begin() + i);
        } else {
            i++;
        }
    }
...
void CSoundRender_Core::_create_data(ref_sound_data& S, LPCSTR fName, esound_type sound_type, int game_type) {
    ...
    S.dwBytesTotal = S.handle->bytes_total(); S.fTimeTotal = S.handle->length_sec();
}
...
CSoundRender_Environment* CSoundRender_Core::get_environment(const Fvector& P) {
    ...
    if (bUserEnvironment) return &s_user_environment;
    ...
    if (geom_ENV) {
        ...
        if (geom_DB.r_count()) {
            ...
            Fvector tri_norm; tri_norm.mknormal(V[T->verts[0]], V[T->verts[1]], V[T->verts[2]]);
            ...
            identity.set_identity(); return &identity;
        }
    } else {
        identity.set_identity(); return &identity;
    }
...
void CSoundRender_Core::set_user_env(CSound_environment* E) {
    ...
    } else bUserEnvironment = FALSE;
    env_apply();
}
void CSoundRender_Core::refresh_env_library() { env_unload(); env_load(); env_apply(); }
void CSoundRender_Core::refresh_sources() {
    for (u32 eit = 0; eit < s_emitters.size(); eit++) s_emitters[eit]->stop(FALSE);
    for (u32 sit = 0; sit < s_sources.size(); sit++) {
        CSoundRender_Source* s = s_sources[sit];
        s->unload(); s->load(*s->fname);
    }
}
