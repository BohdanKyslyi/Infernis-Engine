...
// SIMD-допоміжна функція для швидкого множення транспонованої матриці на вектор (для dDOT14)
ICF __m128 SIMD_Mul_MatrixT_Vector(const dReal* rot, __m128 v_axis) {
    __m128 row0 = _mm_loadu_ps(rot);
    __m128 row1 = _mm_loadu_ps(rot + 4);
    __m128 row2 = _mm_loadu_ps(rot + 8);
    
    __m128 v_x = _mm_shuffle_ps(v_axis, v_axis, _MM_SHUFFLE(0, 0, 0, 0));
    __m128 v_y = _mm_shuffle_ps(v_axis, v_axis, _MM_SHUFFLE(1, 1, 1, 1));
    __m128 v_z = _mm_shuffle_ps(v_axis, v_axis, _MM_SHUFFLE(2, 2, 2, 2));
    
    return _mm_add_ps(_mm_add_ps(_mm_mul_ps(v_x, row0), _mm_mul_ps(v_y, row1)), _mm_mul_ps(v_z, row2));
}
...
static void computeFinalTx(dGeomID geom_transform, dReal* final_pos, dReal* final_R) {
    ...
    // Векторизована математика замість dMULTIPLY0_331
    __m128 v_obj_pos = _mm_loadu_ps(dGeomGetPosition(obj));
    __m128 v_row0 = _mm_loadu_ps(R);
    __m128 v_row1 = _mm_loadu_ps(R + 4);
    __m128 v_row2 = _mm_loadu_ps(R + 8);
    
    final_pos[0] = _mm_cvtss_f32(_mm_dp_ps(v_row0, v_obj_pos, 0x71)) + pos[0];
    final_pos[1] = _mm_cvtss_f32(_mm_dp_ps(v_row1, v_obj_pos, 0x71)) + pos[1];
    final_pos[2] = _mm_cvtss_f32(_mm_dp_ps(v_row2, v_obj_pos, 0x71)) + pos[2];
    
    dMULTIPLY3_333(final_R, R, dGeomGetRotation(obj));
}
...
void GetBoxExtensions(dGeomID box, const dReal* axis, const dReal* pos, const dReal* rot, float center_prg, dReal* lo_ext, dReal* hi_ext) {
    ...
    // Векторизація SIMD (відмова від повільного dDOT та dDOT14)
    __m128 v_axis = _mm_loadu_ps(axis);
    __m128 v_pos  = _mm_loadu_ps(pos);
    
    dReal dif = _mm_cvtss_f32(_mm_dp_ps(v_pos, v_axis, 0x71)) - center_prg;
    
    // Векторизоване обчислення проекцій dDOT14(axis, rot+n)
    __m128 v_dots = SIMD_Mul_MatrixT_Vector(rot, v_axis);
...
void GetCylinderExtensions(dGeomID cyl, const dReal* axis, const dReal* pos, const dReal* rot, float center_prg, dReal* lo_ext, dReal* hi_ext) {
    ...
    // Векторизований розрахунок косинусів
    __m128 v_dots = SIMD_Mul_MatrixT_Vector(rot, v_axis); // [cos1, _cos, cos3, 0]
    
    // Отримуємо абсолютне значення для _cos (індекс 1)
    __m128 v_abs_mask = _mm_castsi128_ps(_mm_set1_epi32(0x7FFFFFFF));
    __m128 v_abs_dots = _mm_and_ps(v_dots, v_abs_mask);
    
    alignas(16) float dots[4];
    _mm_store_ps(dots, v_abs_dots);
...
void TransformedGeometryExtensionLocalParams(dGeomID geom_transform, const dReal* axis, float center_prg, dReal* local_axis, dReal& local_center_prg) {
    ...
    __m128 v_lpos = _mm_loadu_ps(local_pos);
    __m128 v_laxis = _mm_loadu_ps(local_axis);
    
    local_center_prg = center_prg - _mm_cvtss_f32(_mm_dp_ps(v_lpos, v_laxis, 0x71));
}
...
void CODEGeom::clear_motion_history(bool set_unspecified) {
    ...
    if (Fvector().sub(m.c, cast_fv(dGeomGetUserData(geom())->last_pos)).magnitude() > EPS)
        Msg("! [Noir Engine] ПОМИЛКА: Неправильне зміщення (WRONG THING)");
#endif
}
...
void CODEGeom::set_material(u16 ul_material) {
    if (!m_geom_transform) return;
    dGeomID target = geom() ? geom() : m_geom_transform;
    VERIFY(dGeomGetUserData(target));
    dGeomGetUserData(target)->material = ul_material;
}
...
void CODEGeom::set_contact_cb(ContactCallbackFun* ccb) {
    ...
    dGeomID target = geom() ? geom() : m_geom_transform;
    VERIFY(dGeomGetUserData(target));
    dGeomUserDataSetContactCallback(target, ccb);
}
...
void CODEGeom::set_obj_contact_cb(ObjectContactCallbackFun* occb) {
    ...
    dGeomID target = geom() ? geom() : m_geom_transform;
    VERIFY(dGeomGetUserData(target));
    dGeomUserDataSetObjectContactCallback(target, occb);
}
...
void CODEGeom::add_obj_contact_cb(ObjectContactCallbackFun* occb) {
    ...
    dGeomID target = geom() ? geom() : m_geom_transform;
    VERIFY(dGeomGetUserData(target));
    dGeomUserDataAddObjectContactCallback(target, occb);
}
...
void CODEGeom::remove_obj_contact_cb(ObjectContactCallbackFun* occb) {
    ...
    dGeomID target = geom() ? geom() : m_geom_transform;
    VERIFY(dGeomGetUserData(target));
    dGeomUserDataRemoveObjectContactCallback(target, occb);
}
...
void CODEGeom::set_callback_data(void* cd) {
    ...
    dGeomID target = geom() ? geom() : m_geom_transform;
    VERIFY(dGeomGetUserData(target));
    dGeomUserDataSetCallbackData(target, cd);
}
...
void* CODEGeom::get_callback_data() {
    ...
    dGeomID target = geom() ? geom() : m_geom_transform;
    VERIFY(dGeomGetUserData(target));
    return dGeomGetUserData(target)->callback_data;
}
...
void CODEGeom::set_ref_object(IPhysicsShellHolder* ro) {
    ...
    dGeomID target = geom() ? geom() : m_geom_transform;
    VERIFY(dGeomGetUserData(target));
    dGeomUserDataSetPhysicsRefObject(target, ro);
}
...
void CODEGeom::set_ph_object(CPHObject* o) {
    ...
    dGeomID target = geom() ? geom() : m_geom_transform;
    VERIFY(dGeomGetUserData(target));
    dGeomGetUserData(target)->ph_object = o;
}
...
void CODEGeom::init() {
    ...
    dGeomTransformSetGeom(m_geom_transform, g);
    dGeomTransformSetInfo(m_geom_transform, 1);
    dGeomCreateUserData(g);
    dGeomUserDataSetBoneId(g, m_bone_id);
}
...
void CBoxGeom::get_max_area_dir_bt(Fvector& dir) {
    ...
    if (S1 > S2) {
        if (S1 > S3) { ddir[0] = R[2]; ddir[1] = R[6]; ddir[2] = R[10]; } // S1
        else         { ddir[0] = R[0]; ddir[1] = R[4]; ddir[2] = R[8];  } // S3
    } else {
        if (S2 > S3) { ddir[0] = R[1]; ddir[1] = R[5]; ddir[2] = R[9];  } // S2
        else         { ddir[0] = R[0]; ddir[1] = R[4]; ddir[2] = R[8];  } // S3
    }

    if (geom()) {
        const dReal* TR = dGeomGetRotation(geometry_transform());
        __m128 v_ddir = _mm_loadu_ps(ddir);
        __m128 v_row0 = _mm_loadu_ps(TR);
        __m128 v_row1 = _mm_loadu_ps(TR + 4);
        __m128 v_row2 = _mm_loadu_ps(TR + 8);
        
        dir.x = _mm_cvtss_f32(_mm_dp_ps(v_ddir, v_row0, 0x71));
        dir.y = _mm_cvtss_f32(_mm_dp_ps(v_ddir, v_row1, 0x71));
        dir.z = _mm_cvtss_f32(_mm_dp_ps(v_ddir, v_row2, 0x71));
    } else {
        dir.x = ddir[0]; dir.y = ddir[1]; dir.z = ddir[2];
    }
}
