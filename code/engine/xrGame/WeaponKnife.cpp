...
void CWeaponKnife::FastStrike(u32 state) {
    if (state == 0) {
        ...
        m_eHitType = m_eHitType_1;
        fCurrentHit = GameID() == eGameIDSingle ? fvHitPower_1[g_SingleGameDifficulty]
                                                : fvHitPower_1[egdMaster];
        fHitImpulse_cur = fHitImpulse_1;
    } else if (state == 1) {
        ...
        m_perv_hits_count = 0;
        m_eHitType = m_eHitType_2;
        fCurrentHit = GameID() == eGameIDSingle ? fvHitPower_2[g_SingleGameDifficulty]
