...
static void SetWpnParamIconShader(CUIStatic& item, LPCSTR section) {
...
void CUIWpnParams::SetInfo(CInventoryItem* slot_wpn, CInventoryItem& cur_wpn) {
    ...
    if (IsGameTypeSingle()) {
        ...
        SetWpnParamIconShader(m_stAmmoType1, ammo_types[0].c_str());
        Frect tex_rect;
        ...
        if (ammo_types.size() == 1) {
            ...
            SetWpnParamIconShader(m_stAmmoType2, ammo_types[1].c_str());
            tex_rect.x1 =
