...
void CLevel::PrepareToSaveDemo() 
{
    ...
    string_path demo_name = {0};
    string_path demo_path = {0};
    SYSTEMTIME Time;
    GetLocalTime(&Time);
...
#ifdef MP_LOGGING
    if (message_filter* tmp_msg_filter = GetMessageFilter(); tmp_msg_filter) {
        string_path demo_msg_path = {0};
...
bool CLevel::LoadPacket(NET_Packet& dest_packet, u32 global_time_delta) 
{
    ...
    bool condition = map_data.m_sended_map_name_request ? 
    ...
    if (condition) {
        ...
        dest_packet.timeReceive = tmp_hdr.m_timeReceive; 
        dest_packet.r_pos = 0;
...
constexpr float MAX_PLAY_SPEED = 8.f; 
